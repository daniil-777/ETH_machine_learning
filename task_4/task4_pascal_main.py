...
# The complete model contains layers that compare the positions of the three inputs in a space
# generated by the layer 'branch_out'. Here we construct a model that only takes one input and
# only takes it to this space so that we can look at the distribution of data in this space,
# without calculating any distances.
def construct_plotter_model(siamese_model):
	input_shape = (2048,)

	# In Keras, sub models can't be accessed by name it seems. The branch
	# model 'siamese_branch' is at index 3.
	branch = siamese_model.get_layer(index=3)

	def getw(layer_name):
		return branch.get_layer(layer_name).get_weights()

	inp = keras.Input(shape=input_shape)
	inner = keras.layers.Dense(64, name='branch_inner_1', activation='relu', weights=getw('branch_inner_1'))(inp)
	inner = keras.layers.Dense(32, name='branch_inner_2', activation='relu', weights=getw('branch_inner_2'))(inner)
	output = keras.layers.Dense(16, name='branch_out', weights=getw('branch_out'))(inner)
